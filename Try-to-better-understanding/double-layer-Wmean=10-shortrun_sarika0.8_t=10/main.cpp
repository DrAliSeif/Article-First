...
int main() {
    ...
    double* frequency_layer2 = read_initial_1D("W=Natural frequency/Sarika0.8a", int(data[0]));
    for (int i = 0; i < int(data[0]); i++) {
    	frequency_layer1[i]=frequency_layer1[i]+10.0;
    	frequency_layer2[i]=frequency_layer2[i]+10.0;
    }
    ...
    while (Coupling_variable <= (data[8])) { // Coupling loop
        ...
        filesystem::create_directories(name_file_data("Save/Phases/layer2/",data,12));
        ofstream Save_phases1_for_each_coupling(name_file_data("Save/Phases/layer1/",data,12)+"/k="+to_string(Coupling_variable)+".txt");
        ofstream Save_phases2_for_each_coupling(name_file_data("Save/Phases/layer2/",data,12)+"/k="+to_string(Coupling_variable)+".txt");
        ...
        cout<< Coupling_variable << '\t' << Total_synchrony_layer1 << '\t' << Total_synchrony_layer2<<'\t' <<"Execution Time: "<< (double)(end_calculate_time-start_calculate_time)<<" Seconds"<<endl;
        Avg_Sync << Coupling_variable << '\t' << Total_synchrony_layer1<< '\t' << Total_synchrony_layer2<< '\t' << (double)(end_calculate_time-start_calculate_time) << endl;
