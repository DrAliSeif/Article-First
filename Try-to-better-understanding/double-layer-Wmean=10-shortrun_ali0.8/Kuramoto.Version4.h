            ...
            double L)
{
    ...
    M = W + (coupling / (N * 1.0)) * a +L*sin(second_layer-phi+frustration_inter_layer);
    return M;
}
...
{                                
    ...
    for (int i = 0; i < Number_of_node; i++)
    {
        double k1 = dydt(i, data[2], Number_of_node,data[4],coupling, W[i], adj[i], y[i], Phases_history_delay,second_layer[i], data[1]);
        double k2 = dydt(i, data[2], Number_of_node,data[4],coupling, W[i], adj[i], y[i] + k1 * data[4] / 2.0,Phases_history_delay,second_layer[i], data[1]);
        double k3 = dydt(i, data[2], Number_of_node,data[4],coupling, W[i], adj[i], y[i] + k2 * data[4] / 2.0,Phases_history_delay,second_layer[i], data[1]);
        double k4 = dydt(i, data[2], Number_of_node,data[4],coupling, W[i], adj[i], y[i] + k3 * data[4], Phases_history_delay,second_layer[i], data[1]);
...
int write_last_phase(string address,double* data,int Number_of_row,double* last_Phase_layer) {
    ofstream file_print(name_file_data(address,data,12)+".txt");
    for (int i = 0; i < int(data[0]); i++) {
        file_print << last_Phase_layer[i] << endl;
    }
    file_print.close();
    return 0;
}
